...
UserDefaults stores destination order, hidden destination IDs, optional user-assigned nicknames keyed by stable destination ID, hover delay, navigation inclusion, picker appearance/size, and welcome state. IDs for missing profiles are retained to preserve preferences across temporary unavailability. Nicknames affect presentation only and never browser launch routing. Login items use `SMAppService`, off by default. Troubleshooting is session-only and off by default. No hovered URL is persisted by PickBrowser; explicitly copied URLs remain on the system clipboard. Sparkle owns its own update preferences; see [release/update architecture](RELEASING.md).
