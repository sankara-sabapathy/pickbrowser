...
![PickBrowser opening beside a hovered webpage link with copy, browser-tab, and browser-profile choices](docs/images/pickbrowser-sneak-peek.png)

*A sneak peek of PickBrowser over an example webpage. Profile names shown are examples.*
...
On first installed launch, click **Enable Accessibility** and enable PickBrowser under **System Settings → Privacy & Security → Accessibility**. Return to the app; detection starts automatically. Keeping the app in Applications gives macOS a stable path for Accessibility permission and updates. If access was previously granted but is no longer valid, Settings displays an orange **Accessibility needs renewal** card with the exact recovery steps. Do not remove and re-add permission unless that warning is visible. Current releases are ad-hoc signed and not Apple-notarized, so an update can change the code identity macOS associates with the approval; stable Developer ID signing is needed to make update retention reliable.
...
The menu-bar menu contains Pause/Resume, Settings, and Quit. Settings lets you hide/reorder destinations, give each destination a nickname, resize the picker from 80% to 140%, set a custom hover delay from 0.1 to 5 seconds (default 0.5), and opt into launch at login or update checks. Press Return after editing a nickname to dismiss the field; a green checkmark confirms the saved value. Move outside both the link and picker for 250 ms to dismiss; the brief grace allows crossing into the picker. Troubleshooting is off by default and can be enabled for the current session. All destinations are initially visible, with a stable order. Profiles temporarily missing from disk retain their saved order, nickname, and visibility preferences.

PickBrowser marks its picker, settings, and alerts as non-shareable so capture tools that honor macOS window-sharing hints omit them. [Apple now treats that mechanism as legacy](https://developer.apple.com/documentation/appkit/nswindow/sharingtype-swift.enum/none), and a full-display share can still capture the composited window. Use **Pause** before sharing when the picker must never appear. PickBrowser does not request Screen Recording access or inspect other applications' capture sessions.
...
Only Accessibility access is requested. There is no screen capture, OCR, clipboard reading, browser extension, account, telemetry, link history, or default-browser registration. Screen-share shielding is a best-effort window attribute and does not require Screen Recording access. Copy writes the destination to the clipboard only when clicked. URLs otherwise exist transiently while detecting, presenting, or opening a link. Destination ordering/visibility/nicknames, hover delay, navigation filtering, appearance and size, welcome state, and update preferences are stored locally. Login registration is opt-in through macOS. Optional update checks contact GitHub; hovered URLs and browser profile data are never included.
