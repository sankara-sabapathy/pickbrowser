...
The menu-bar menu contains Pause/Resume, Settings, and Quit. Settings lets you hide/reorder destinations, give each destination a nickname, resize the picker from 80% to 140%, set a custom hover delay from 0.1 to 5 seconds (default 0.5), and opt into launch at login or update checks. Move outside both the link and picker for 250 ms to dismiss; the brief grace allows crossing into the picker. Troubleshooting is off by default and can be enabled for the current session. All destinations are initially visible, with a stable order. Profiles temporarily missing from disk retain their saved order, nickname, and visibility preferences.
